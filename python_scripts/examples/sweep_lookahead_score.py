...
def main():
    ...
    configs = [
        # score_family = 1 (SCORE_MIN_CANDIDATES), criterion = 1 (SELECT_MIN)
        {"sf": 1, "crit": 1, "ur": 0.40, "pd": 30},
        {"sf": 1, "crit": 1, "ur": 0.40, "pd": 15},
        {"sf": 1, "crit": 1, "ur": 0.50, "pd": 15},
        {"sf": 1, "crit": 1, "ur": 0.50, "pd": 30},
        
        # score_family = 2 (SCORE_PROGRESS), criterion = 0 (SELECT_MAX)
        {"sf": 2, "crit": 0, "ur": 0.40, "pd": 15},
        {"sf": 2, "crit": 0, "ur": 0.40, "pd": 30},
        {"sf": 2, "crit": 0, "ur": 0.50, "pd": 15},
        
        # original branching score_family = 0
        {"sf": 0, "crit": 0, "ur": 0.40, "pd": 30},
        {"sf": 0, "crit": 0, "ur": 0.40, "pd": 15},
        {"sf": 0, "crit": 0, "ur": 0.50, "pd": 15},
    ]
    
    print(f"| ScoreFamily | Criterion | G_MIN_UNSET_R_PRUNE | DEEP | Ref Nodes | Base Nodes | Node Red % | Ref Time | Base Time | Speedup % |")
    print(f"|---|---|---|---|---|---|---|---|---|---|")
    
    for cfg in configs:
        sf, crit, ur, pd = cfg["sf"], cfg["crit"], cfg["ur"], cfg["pd"]
        cflags = f"-Wall -Wextra -Werror -O2 -DLOOKAHEAD_SCORE_FAMILY={sf} -DLOOKAHEAD_CRITERION={crit} -DG_MIN_UNSET_R_PRUNE={ur} -DREBUILD_PERIOD=16 -DG_PRUNE_PERIOD_SHALLOW=16 -DG_PRUNE_EXTRA_PERIOD_DEEP={pd} -DREBUILD_IN_LOOKAHEAD=0"
        ...
        print(f"| {sf} | {crit} | {ur:.2f} | {pd} | {ref_nodes} | {base_nodes} | {node_red:.2f}% | {ref_time:.3f}s | {base_time:.3f}s | {speedup:.2f}% |")
