...
def main():
    ...
    unset_ratios = [0.30, 0.35, 0.40, 0.45, 0.50, 0.55]
    
    print(f"| G_MIN_UNSET_R_PRUNE | Ref Nodes | Base Nodes | Node Red % | Ref Time | Base Time | Speedup % |")
    print(f"|---|---|---|---|---|---|---|")
    
    for ur in unset_ratios:
        cflags = f"-Wall -Wextra -Werror -O2 -DG_MIN_UNSET_R_PRUNE={ur} -DREBUILD_PERIOD=4 -DG_PRUNE_PERIOD_SHALLOW=16 -DG_PRUNE_EXTRA_PERIOD_DEEP=15 -DREBUILD_IN_LOOKAHEAD=0"
        run_cmd("make clean")
        ...
        print(f"| {ur:.2f} | {ref_nodes} | {base_nodes} | {node_red:.2f}% | {ref_time:.3f}s | {base_time:.3f}s | {speedup:.2f}% |")
