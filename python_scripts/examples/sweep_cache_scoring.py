...
def main():
    ...
    configs = [
        # 1. Base case (Force invalidate = 1, use constrs scoring = 0, disable gac = 0, prune level = 1)
        {"fic": 1, "ucs": 0, "gac": 0, "lvl": 1},
        
        # 2. Test force invalidate = 0
        {"fic": 0, "ucs": 0, "gac": 0, "lvl": 1},
        {"fic": 0, "ucs": 0, "gac": 1, "lvl": 1},
        {"fic": 0, "ucs": 0, "gac": 1, "lvl": 0},
        
        # 3. Test use constrs scoring = 1
        {"fic": 1, "ucs": 1, "gac": 0, "lvl": 1},
        {"fic": 0, "ucs": 1, "gac": 1, "lvl": 1},
        {"fic": 0, "ucs": 1, "gac": 1, "lvl": 0},
    ]
    
    print(f"| Invalidate | ConstrsScoring | GAC_Disabled | PruneLevel | Ref Nodes | Base Nodes | Node Red % | Ref Time | Base Time | Speedup % |")
    print(f"|---|---|---|---|---|---|---|---|---|---|")
    
    for cfg in configs:
        fic, ucs, gac, lvl = cfg["fic"], cfg["ucs"], cfg["gac"], cfg["lvl"]
        cflags = f"-Wall -Wextra -Werror -O2 -DFORCE_INVALIDATE_CACHE={fic} -DUSE_CONSTRS_SCORING={ucs} -DDISABLE_GAC={gac} -DG_LOOKAHEAD_PRUNING_LEVEL={lvl} -DG_MIN_UNSET_R_PRUNE=0.40 -DREBUILD_PERIOD=16 -DG_PRUNE_PERIOD_SHALLOW=16 -DG_PRUNE_EXTRA_PERIOD_DEEP=15 -DREBUILD_IN_LOOKAHEAD=0"
        ...
        print(f"| {fic} | {ucs} | {gac} | {lvl} | {ref_nodes} | {base_nodes} | {node_red:.2f}% | {ref_time:.3f}s | {base_time:.3f}s | {speedup:.2f}% |")
