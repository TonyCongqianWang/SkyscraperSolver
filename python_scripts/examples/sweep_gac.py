...
def main():
    ...
    configs = [
        # GAC enabled, pruning level = 1
        {"gac": 0, "lvl": 1, "ur": 0.40, "pd": 15, "rp": 16},
        # GAC disabled, pruning level = 1
        {"gac": 1, "lvl": 1, "ur": 0.40, "pd": 15, "rp": 16},
        
        # GAC enabled, pruning level = 0
        {"gac": 0, "lvl": 0, "ur": 0.40, "pd": 15, "rp": 16},
        # GAC disabled, pruning level = 0
        {"gac": 1, "lvl": 0, "ur": 0.40, "pd": 15, "rp": 16},
    ]
    
    print(f"| GAC_Disabled | PruneLevel | G_MIN_UNSET_R_PRUNE | DEEP | REBUILD_PERIOD | Ref Nodes | Base Nodes | Node Red % | Ref Time | Base Time | Speedup % |")
    print(f"|---|---|---|---|---|---|---|---|---|---|---|")
    
    for cfg in configs:
        gac, lvl, ur, pd, rp = cfg["gac"], cfg["lvl"], cfg["ur"], cfg["pd"], cfg["rp"]
        cflags = f"-Wall -Wextra -Werror -O2 -DDISABLE_GAC={gac} -DG_LOOKAHEAD_PRUNING_LEVEL={lvl} -DG_MIN_UNSET_R_PRUNE={ur} -DREBUILD_PERIOD={rp} -DG_PRUNE_PERIOD_SHALLOW=16 -DG_PRUNE_EXTRA_PERIOD_DEEP={pd} -DREBUILD_IN_LOOKAHEAD=0"
        ...
        print(f"| {gac} | {lvl} | {ur:.2f} | {pd} | {rp} | {ref_nodes} | {base_nodes} | {node_red:.2f}% | {ref_time:.3f}s | {base_time:.3f}s | {speedup:.2f}% |")
