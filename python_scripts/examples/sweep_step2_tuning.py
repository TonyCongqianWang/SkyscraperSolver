...
def evaluate_solvers(benchmark_file, options, step2_binary):
    ...
    # We will test baseline, step1, and the compiled step2 on the exact same dataset
    results = {
...
def main():
    ...
    # We will sweep:
    # 1. REBUILD_PERIOD (rp): 8, 12, 16, 20
    # 2. G_PRUNE_EXTRA_PERIOD_DEEP (pd): 10, 15, 20
    # 3. G_MIN_UNSET_R_PRUNE (ur): 0.35, 0.40, 0.45
    ...
    print(f"| REBUILD_PERIOD | DEEP_PERIOD | UNSET_RATIO | Base Nodes | Step1 Nodes | Step2 Nodes | Base Time | Step1 Time | Step2 Time | Step2 NodeRed % | Step2 Speedup % |")
    print(f"|---|---|---|---|---|---|---|---|---|---|---|")
    
    for cfg in sweeps:
        rp, pd, ur = cfg["rp"], cfg["pd"], cfg["ur"]
        cflags = f"-Wall -Wextra -Werror -O2 -DREBUILD_PERIOD={rp} -DG_PRUNE_EXTRA_PERIOD_DEEP={pd} -DG_MIN_UNSET_R_PRUNE={ur} -DG_PRUNE_PERIOD_SHALLOW=16 -DREBUILD_IN_LOOKAHEAD=0"
        ...
        print(f"| {rp} | {pd} | {ur:.2f} | {base_nodes} | {step1_nodes} | {step2_nodes} | {base_time:.3f}s | {step1_time:.3f}s | {step2_time:.3f}s | {node_red:.2f}% | {speedup:.2f}% |")
