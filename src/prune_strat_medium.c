...
{
	lim->gac_min_entropy = g_medium_gac_min_entropy;
	lim->constr_min_entropy = g_medium_constr_min_entropy;
	lim->lh_continue_min_entropy = g_medium_lookahead_continue_min_entropy;
	lim->lh_continue_slope = g_medium_lookahead_continue_slope;
	lim->gac_local_min_entropy = g_medium_gac_local_min_entropy;
	lim->gac_local_max_entropy = g_medium_gac_local_max_entropy;
	lim->gac_global_min_entropy = g_medium_gac_global_min_entropy;
	lim->constr_local_min_entropy = g_medium_constr_local_min_entropy;
	lim->constr_local_max_entropy = g_medium_constr_local_max_entropy;
	lim->constr_global_min_entropy = g_medium_constr_global_min_entropy;
...
int	prune_strat_medium(t_puzzle *puzzle)
{
	...
	if (node->is_invalid || node->is_complete || node->num_unset == 0
		|| node->remaining_entropy < g_medium_min_entropy)
		return (0);
