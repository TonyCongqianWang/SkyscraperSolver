...
{
	lim->gac_min_entropy = g_shallow_gac_min_entropy;
	lim->constr_min_entropy = g_shallow_constr_min_entropy;
	lim->lh_continue_min_entropy = g_shallow_lookahead_continue_min_entropy;
	lim->lh_continue_slope = g_shallow_lookahead_continue_slope;
	lim->gac_local_min_entropy = g_shallow_gac_local_min_entropy;
	lim->gac_local_max_entropy = g_shallow_gac_local_max_entropy;
	lim->gac_global_min_entropy = g_shallow_gac_global_min_entropy;
	lim->constr_local_min_entropy = g_shallow_constr_local_min_entropy;
	lim->constr_local_max_entropy = g_shallow_constr_local_max_entropy;
	lim->constr_global_min_entropy = g_shallow_constr_global_min_entropy;
	lim->lh_constr_local_min_entropy
		= g_shallow_lookahead_constr_local_min_entropy;
	lim->lh_constr_local_max_entropy
		= g_shallow_lookahead_constr_local_max_entropy;
	lim->lh_constr_global_min_entropy
		= g_shallow_lookahead_constr_global_min_entropy;
	lim->lh_gac_local_min_entropy
		= g_shallow_lookahead_gac_local_min_entropy;
	lim->lh_gac_local_max_entropy
		= g_shallow_lookahead_gac_local_max_entropy;
	lim->lh_gac_global_min_entropy
		= g_shallow_lookahead_gac_global_min_entropy;
}
...
static double	calc_period(t_puzzle *puzzle, t_node_state *node)
{
	...
	return (g_shallow_period_coef_scale * dpow075_approx(raw)
		+ g_shallow_period_coef_unset
...
int	prune_strat_shallow(t_puzzle *puzzle)
{
	...
	if (node->is_invalid || node->is_complete || node->num_unset == 0
		|| node->remaining_entropy < g_shallow_min_entropy)
		return (0);
