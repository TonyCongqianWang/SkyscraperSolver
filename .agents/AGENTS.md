...
* **No Absolute Paths**: Agents must never hardcode machine-specific absolute file paths (such as `C:\Users\...\SkyscraperSolver` or `/home/username/...`).
...
* **Walkthroughs**: Walkthroughs documenting active refactoring steps or testing results must be placed inside the `scratch/` directory (e.g., `scratch/walkthrough_refactor.md`) rather than in tracked root folders.
* **Git Excluded**: The `scratch/` directory is git-ignored, ensuring these files are never staged or committed.

---

## 📐 5. Code Standard & Verification Requirements

* **Norminette Compliance**: All C files in `src/` must adhere strictly to 42 Norminette rules (`norminette src/`) with **0 errors**.
  - Max 5 functions per `.c` file.
  - Max 25 lines per function body.
  - No consecutive blank lines.
* **Zero External Math Dependencies**: Solver C code (`src/`) must not import `<math.h>` or use external C math functions.
* **Mandatory Verification Routine**: After making C code changes, always verify with:
  1. `norminette src/`
  2. `make re`
  3. `micromamba run -n solver make test`
  4. `micromamba run -n solver bash setup_and_tune.sh`
